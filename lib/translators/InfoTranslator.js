...
const InfoTranslator = {
    ...
    warToJson: function(buffer) {
        ...
        if (editorVersion > 6061) {
            // 16 Reforged mystery bytes
        ...
        if (editorVersion > 6072) {
            // 12 Reforged mystery bytes
            for (let di = 0; di < 3; ++di) {
                b.readInt();
            }
        } else if (editorVersion > 6061) {
            // 4 Semi-forged mystery bytes
            b.readInt();
        }
        // Struct: players
